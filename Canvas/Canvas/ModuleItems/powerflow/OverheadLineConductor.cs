...
{
    class overhead_line_conductor : Module
    {

        public overhead_line_conductor() { setupProperties(new List<Property>()); }
        public overhead_line_conductor(overhead_line_conductor old) { m_p1 = old.FromPoint; m_p2 = old.StartPoint; m_p3 = old.EndPoint; m_p4 = old.ToPoint; setupProperties(old.Properties); }
        ...
        {
            wr.WriteStartElement("overhead_line_conductor");
        ...
        {
            return string.Format("overhead_line_conductor@{0},{1}",
                StartPoint.PosAsString(),
                EndPoint.PosAsString());
        }
        public override IDrawObject Clone()
        {
            return new overhead_line_conductor(this);
        }
        public override string toGLM()
        {
            String s = "object overhead_line_conductor {" + System.Environment.NewLine;
        ...
        {
            get { return "overhead_line_conductor"; }
        }
        public override void Draw(ICanvas canvas, RectangleF unitrect)
        {
            ...
            canvas.Graphics.DrawString("OHL Conductor", m_font, B, canvas.ToScreen(new UnitPoint(m_p1.X - 0.4, m_p1.Y + 0.2)).X, canvas.ToScreen(new UnitPoint(m_p1.X - 0.3, m_p1.Y + 0.2)).Y);
