...
{
    class triplex_line_configuration : ModuleItems.Module
    {

        public triplex_line_configuration() { setupProperties(new List<Property>()); }
        public triplex_line_configuration(triplex_line_configuration old) { m_p1 = old.StartPoint; m_p2 = old.EndPoint; m_p3 = old.ToPoint; setupProperties(old.Properties); }
        ...
        {
            wr.WriteStartElement("triplex_line_configuration");
        ...
        {
            return string.Format("triplex_line_configuration@{0}",
                StartPoint.PosAsString());
        }
        public override IDrawObject Clone()
        {
            return new triplex_line_configuration(this);
        }
        public override string toGLM()
        {
            String s = "object triplex_line_configuration {" + System.Environment.NewLine;
        ...
        {
            get { return "triplex_line_configuration"; }
        }
        public override void Draw(ICanvas canvas, RectangleF unitrect)
        {
            ...
            canvas.Graphics.DrawString("TpLine Config", m_font, B, canvas.ToScreen(new UnitPoint(m_p1.X - 0.4, m_p1.Y + 0.2)).X, canvas.ToScreen(new UnitPoint(m_p1.X - 0.3, m_p1.Y + 0.2)).Y);
