...
namespace Canvas.ModuleItems.powerflow
{
    class capacitor : Module
    {
        ...
        public capacitor() { setupProperties(new List<Property>()); this.child = true; }
        public capacitor(capacitor old) { m_p1 = old.FromPoint; m_p2 = old.StartPoint; m_p3 = old.EndPoint; m_p4 = old.ToPoint; setupProperties(old.Properties); this.child = true; }
        ...
        {
            wr.WriteStartElement("capacitor");
        ...
        {
            return string.Format("capacitor@{0}",
        ...
        {
            String s = "object capacitor {" + System.Environment.NewLine;
        ...
        public override void Draw(ICanvas canvas, RectangleF unitrect)
        {
            ...
            if (horizontal)
            {
                ...
                canvas.Graphics.DrawRectangle(pen, canvas.ToScreen(new UnitPoint(m_p2.X + 0.3, 1)).X, canvas.ToScreen(new UnitPoint(1, m_p2.Y + 0.1)).Y, canvas.ToScreen(m_p3).X - canvas.ToScreen(new UnitPoint(m_p2.X + 0.3, 1)).X, canvas.ToScreen(new UnitPoint(1, m_p2.Y - 0.1)).Y - canvas.ToScreen(new UnitPoint(1, m_p2.Y + 0.1)).Y);
            }
            else
            {
