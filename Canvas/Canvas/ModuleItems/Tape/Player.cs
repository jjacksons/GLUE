...
namespace Canvas.ModuleItems.tape
{
    class player : Module
    {
        ...
        public player() { setupProperties(new List<Property>()); this.child = true; }
        public player(player old) { m_p1 = old.FromPoint; m_p2 = old.StartPoint; m_p3 = old.EndPoint; m_p4 = old.ToPoint; setupProperties(old.Properties); this.child = true; }
        private void setupProperties(List<Property> prop)
        {
            ...
            DefaultProperties.Add(new Property("loop", "", ""));
            DefaultProperties.Add(new Property("DELTAMODE", "", "VA"));
        ...
        {
            String s = "object player {" + System.Environment.NewLine;
        ...
        public override void Draw(ICanvas canvas, RectangleF unitrect)
        {
            ...
            if (horizontal)
            {
                ...
                canvas.Graphics.DrawString("Player", m_font, B, canvas.ToScreen(new UnitPoint(m_p2.X +0.3, m_p2.Y - 0.1)).X, canvas.ToScreen(new UnitPoint(m_p2.X - 0.3, m_p2.Y +0.1)).Y);
            ...
            else
            {
                ...
                float size = (canvas.ToScreen(new UnitPoint(m_p1.X + 1, m_p1.Y)).X - canvas.ToScreen(m_p1).X) / 10;
                Font m_font = new System.Drawing.Font("Arial Black", size, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(0)));
                canvas.Graphics.DrawString("Player", m_font, B, canvas.ToScreen(new UnitPoint(m_p2.X -0.25, 1)).X, canvas.ToScreen(new UnitPoint(1, m_p2.Y -0.2)).Y);
            }
